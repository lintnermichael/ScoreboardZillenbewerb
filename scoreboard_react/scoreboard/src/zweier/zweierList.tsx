...
import EinerAddDialog from './zweierAddDialoge.tsx';

function ZweierList() {
    const [einer, setEiner] = useState<Zweier[]>([]);
    ...
    const saveToDatabase = (zwerier: Zweier) => {
        setSavingStatus(prev => ({ ...prev, [zwerier._id]: true }));
        console.log(JSON.stringify(zwerier));
        fetch(import.meta.env.VITE_API_URL + '/zweier/'+zwerier._id, {
            method: 'PUT',
            body: JSON.stringify({
                name1: zwerier.name1,
                name2: zwerier.name2,
                zeit: zwerier.zeit,
                fehlerpunkte: zwerier.fehlerpunkte,
                feuerwehr: zwerier.feuerwehr,
                disqualiviziert: zwerier.disqualiviziert,
                startnummer: zwerier.startnummer,
            }),
            headers: { 'Content-Type': 'application/json' },
        }).finally(() => {
            setSavingStatus(prev => ({ ...prev, [zwerier._id]: false }));
        });
    };

    const debouncedSaveDisqualiviziert = useMemo(
    () => debounce((einer: Zweier, value: boolean) => {
    ...
    const handleDisqualiviziertChange = (einer: Zweier, checked: boolean) => {
    ...
    const debouncedSaveTime = useMemo(
        () => debounce((einer: Zweier, time: string) => {
    ...
    const debouncedSaveFehlerpunkte = useMemo(
        () => debounce((einer: Zweier, punkte: number) => {
    ...
    const handleTimeChange = (einer: Zweier, value: string) => {
    ...
    const handleFehlerpunkteChange = (einer: Zweier, value: string) => {
    ...
    return (
        <>
            <Button variant="contained" startIcon={<AddIcon />}  onClick={() => setOpenDialog(true)}>Zweier</Button>
                ...
                <Accordion key={item._id}>
                    ...
                    >
                        <Typography component="span" sx={{color: inputDisqualiviziert[item._id] ? colors.red[500] : 'inherit'}}>
                            {inputDisqualiviziert[item._id] ? "DS - " : ""} {item.startnummer} - {item.name1}, {item.name2} {inputDisqualiviziert[item._id] ? "" : "- "+ fullTime[item._id]}
...
function timeSort(data: Zweier[]): Zweier[] {
